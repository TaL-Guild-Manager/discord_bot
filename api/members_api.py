...
async def members(message, filter=None):
    async with aiohttp.ClientSession() as session:
        try:
            ...
            await default_request(url, session, message, members_serializer, error_message, api_error_message)
...
async def member(message):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            await default_request(url, session, message, member_serializer, error_message, api_error_message)
        except aiohttp.ClientError as e:
            await message.channel.send(f"Une erreur de client est survenue: {str(e)}")
        except Exception as e:
            await message.channel.send(f"Une erreur est survenue: {str(e)}")
